...
type UserQuery struct {
	...
	withAuthProviders *AuthProviderQuery
...
// QueryAuthProviders chains the current query on the "auth_providers" edge.
func (_q *UserQuery) QueryAuthProviders() *AuthProviderQuery {
	query := (&AuthProviderClient{config: _q.config}).Query()
...
// WithAuthProviders tells the query-builder to eager-load the nodes that are connected to
// the "auth_providers" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAuthProviders(opts ...func(*AuthProviderQuery)) *UserQuery {
	query := (&AuthProviderClient{config: _q.config}).Query()
...
func (_q *UserQuery) loadAuthProviders(ctx context.Context, query *AuthProviderQuery, nodes []*User, init func(*User), assign func(*User, *AuthProvider)) error {
	...
	for _, n := range neighbors {
		fk := n.user_auth_providers
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_auth_providers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_auth_providers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
